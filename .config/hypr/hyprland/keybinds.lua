...
--##! Window
--# Focusing
hl.bind("SUPER + z", hl.dsp.window.drag(), { mouse = true, description = "Window: Move" }) -- edited mouse:272 -> z
...
-- #/# bind = SUPER+ALT, Page_↑/↓,, -- Send to workspace left/right
for i = 1, 2 do
    local keydirs = { "Up", "Down" }
    local prefix = { "r-", "r+" }
    local descdir = { "left", "right" }
    hl.bind("SUPER + ALT + Page_" .. keydirs[i], hl.dsp.window.move({ workspace = prefix[i] .. "1" }), {description = "Window: Send to workspace " .. descdir[i]})
end
for i = 1, 4 do
    local key = { "SUPER + ALT + Page_", "CTRL + SUPER + ALT + " }
...
hl.bind("SUPER + ALT + O",
    hl.dsp.window.move({ workspace = "special:special_", follow = false }), { description = "Window: Send to scratchpad" })
...
for i = 1, 2 do
    local keys = { "Left", "Right" }
    local prefix = { "m-", "m+" }
    hl.bind("CTRL + SUPER + SHIFT + " .. keys[i], hl.dsp.focus({ workspace = prefix[i] .. "1" })) -- Edited ALT -> SHIFT
end
...
hl.bind("SUPER + O", hl.dsp.workspace.toggle_special("special_"), { description = "Workspace: Toggle scratchpad" })
...
hl.bind("SUPER + K", hl.dsp.exec_cmd("hyprctl switchxkblayout current next"))
